rankhospital <- function(st_abrev, outcome, num = "best") {
	...
	## Read outcome data
	ranks <- read.csv("datasets/outcome-of-care-measures.csv", na.strings = "Not Available")
	print(head(ranks))
	...
	rank.c <- arrange(filter(select(ranks, hosp.h = Hospital.Name, state.h = State, outcome.h = starts_with("Hospital.30.Day.Death") & ends_with(outcome.t)), state.h == st_abrev, outcome.h != "NA"), outcome.h, hosp.h)
